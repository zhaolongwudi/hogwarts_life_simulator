# HogwartLige

> 霍格沃茨 AI 人生模拟器 · 由多模型 AI 驱动的文字冒险游戏

**HogwartLige** 是一款基于 Flutter 构建的高自由度霍格沃茨人生模拟器。你将作为一名巫师踏入魔法世界，从分院帽仪式开始，经历课堂学习、社交互动、剧情事件，最终成长为一名合格的巫师。所有剧情、对话和事件均由 AI 实时生成，每一次选择都会影响你的命运。

## ✨ 核心特性

### 🎭 角色系统
- **自由创建巫师**：姓名、血统（纯血/混血/麻瓜）、学院、出生地、性格特质
- **六大核心属性**：容貌、体质、智力、魅力、体能、道德值
- **动态关系网络**：与 NPC 互动改变好感度（敌对 → 冷淡 → 初识 → 朋友 → 亲密 → 挚友）

### 🌍 世界探索
- **四大区域地图**：霍格沃茨城堡（18 地点）、伦敦（9 地点）、住宅区（8 地点）、霍格莫德村（12 地点）
- **对角巷子地图**：16 家真实魔法店铺（古灵阁、奥利凡德魔杖店、韦斯莱魔法把戏坊等）
- **翻倒巷子地图**：6 个黑魔法地点（博金·博克、黑魔法市集等）
- **层级导航**：大世界 → 子区域 → 更深层子区域，面包屑路径
- **动态 NPC**：14+ 原著角色，各有独立性格、位置和行为

### 🤖 AI 驱动叙事
- **支持多模型**：DeepSeek、智谱 AI（Zhipu）、Agnes-2.5-flash
- **AI 驱动叙事**：每次行动都由 AI 生成独特叙事（三重保障确保叙事永不丢失：结构化解析 → 智能提取 → 兜底生成）
- **自定义行动**：自由输入想法，AI 即时响应
- **上下文记忆**：AI 会记住你的选择和关系变化
- **五色富文本渲染**：剧情中的人名（金）、对话（蓝）、地点（绿）、物品（紫）自动高亮，旁白浅灰色，深色主题下清晰可读

### 📱 四大功能模块
| 模块 | 说明 |
|------|------|
| 📖 剧情 | AI 叙事文本 + 选项 + 属性面板（三重保障确保叙事永不丢失） |
| 📱 手机 | 魔法通讯（AI对话）、魔法论坛、日记、平行世界小剧场、姻缘红娘 |
| 🌍 世界 | NPC 关系网、世界地图、位置导航 |
| ⚙️ 设置 | AI 引擎配置、阅读速度、Token 统计 |

### 🎭 魔法通讯系统（AI 驱动）
- **真实 NPC 对接**：从游戏注册表自动加载联系人，支持学院筛选
- **AI 智能对话**：与任何 NPC 进行个性化 AI 对话，根据性格、好感度调整语气
- **好感度联动**：对话自动影响好感度变化（-100 到 +100）
- **对话历史持久化**：所有对话保存到本地，下次继续
- **11 级关系标签**：死敌→宿怨→反感→冷漠→中立→好感→友好→信任→亲密→深爱→灵魂伴侣

### 📖 日记系统
- 自动从游戏状态生成初始日记条目
- 手动写日记支持 8 种心情表情
- 区分系统生成和用户手写条目

### 🎭 平行世界·小剧场
- 预置"如果"场景（搞笑担当分院帽、魁地奇无扫把等）
- 支持创建自定义小剧场

### 💞 姻缘一线牵红娘
- NPC 智能匹配算法：学院+年级+性格+好感度综合评分
- 显示匹配原因（同院情谊/性格互补/高度契合等）

### 🎮 辅助系统
- **回忆系统**：章节时间线、收藏、CG 画廊
- **职业系统**：找点活干、赚取加隆、消耗体力
- **魔法商店**：淘货、卖闲置、魔法道具
- **本地存档**：自动存档 + 手动存档 + 数据导出

### 🎨 剧情富文本高亮系统
- **五色自动渲染**：`StoryTextRenderer` 智能识别剧情文本中的关键元素并着色
  - 👤 人名 → 明亮金色 `#E3B341`
  - 💬 对话 → 明亮蓝色 `#58A6FF`（支持「」、""、『』等引号）
  - 📍 地点 → 明亮绿色 `#56D364`
  - ⚗️ 物品 → 明亮紫色 `#BC8CFF`
  - 📖 旁白 → 浅灰色 `#C9D1D9`
- **内置词典**：70+ 人名、50+ 地点、40+ 物品关键词
- **智能 token 拆分**：对话优先定位，避免与关键词冲突
- **深色主题优化**：所有文字颜色适配深色背景，确保高对比度可读性

## 🛠 技术栈

| 类别 | 技术 |
|------|------|
| 框架 | Flutter 3.16.0+ |
| 语言 | Dart 3.2.0+ |
| 状态管理 | Provider |
| 本地存储 | SharedPreferences |
| AI 引擎 | DeepSeek / Zhipu / Agnes |
| 构建 | GitHub Actions（自动版本递增） |

## 🚀 快速开始

### 环境要求
- [Flutter SDK](https://docs.flutter.dev/get-started) >= 3.16.0
- JDK 17（用于 Android 构建）
- 至少一个 AI 提供商的 API Key

### 安装运行

```bash
# 克隆项目
git clone https://github.com/zhaolongwudi/hogwarts_life_simulator.git
cd hogwarts_life_simulator

# 安装依赖
flutter pub get

# 运行（需连接设备或模拟器）
flutter run
```

### 配置 AI

首次运行后，在 **设置** 页面配置 AI 引擎：
1. 选择 AI 提供商（DeepSeek / 智谱AI / Agnes）
2. 填入 API Key
3. 选择模型
4. 开始你的魔法之旅

## 📦 构建 APK

### 本地构建

```bash
flutter build apk --release
```

### CI 自动构建

推送代码到 `main` 分支后，GitHub Actions 会自动：
1. **自动递增版本号**（0.0.6 → 0.0.7 → ... → 0.0.10 → 0.1.0）
2. 运行 `flutter analyze` 代码检查
3. 构建 release APK
4. 重命名为 `HogwartLige-{版本号}.apk`
5. 上传为 GitHub Release 资产

下载最新版本：[Releases 页面](https://github.com/zhaolongwudi/hogwarts_life_simulator/releases)

## 🔐 隐私说明

- 本应用**完全本地运行**，AI API Key 仅存储在本地设备
- 不收集任何用户数据，不进行远程上报
- 所有存档数据保存在本地 SharedPreferences
- 实际 AI 服务费用由您选择的提供商收取

## 📝 更新日志

### v0.4.2 — 2026-08-21

**📋 变更说明**
feat: 整合75章终极世界观设定文档

- 新建 world_rules.dart：精编75章完整设定为AI系统提示词常量
  * 包含原著优先级别、世界结构、历史、九大文明支柱等全部核心规则
  * 启动界面选项数据（8时代/11血统/11出生身份/6魔法资质/6学院倾向/6政治倾向/6模拟风格）
  * 九大文明支柱、五层世界结构、魔法等级阶梯、施法失败率、区域危险度、终极原则等常量

### v0.4.1 — 2026-08-20

**📋 变更说明**
feat: 优化界面布局，完善手机功能

- 移除游戏界面顶部和面板中的重复展开/收起箭头（保留面板中的属性展开按钮）
- 移除手机标签页内重复的「打开手机」按钮（与底部导航功能重复）
- 完善手机主屏幕，扩展为 4 行共 12 个应用：
  * 新增第 2 行：对角巷商店、世界地图、平行世界
  * 新增第 3 行：姻缘红娘、找工作、相册回忆

### v0.4.0 — 2026-08-20

**📋 变更说明**
feat: UI layout improvements + AI provider fix + changelog script enhancement

- Swap 事件/面板 tab order (事件 first)
- Auto-scroll to top on narrative update
- Move 推进剧情 button to left of input box
- Fix 智谱 AI baseUrl (use /api/paas/v4/ path)
- Fix Agnes AI baseUrl (remove duplicate /v1)

### v0.3.9 — 2026-08-20

**📋 变更说明**
fix: remove unnecessary foundation.dart import

### v0.3.8 — 2026-08-20

fix: auto-save/auto-load game state - fix black screen on app restart

- Add autoSave/loadAutoSave/clearAutoSave to SaveService
- Add auto-load in GameProvider constructor when isGameStarted=true
- Add auto-save after initializeGame, processChoice, loadFromSave
- Add auto-save on NPC affection change and app dispose
- Add isInitializing state to show loading indicator
- Update GameScreen to show loading/error states during init
- Fix null-safety for narrative/choices in loadFromSave

### v0.3.7 — 2026-08-20

fix: sync README.md with latest features and fix changelog automation

### v0.3.6 — 2026-08-20

feat: auto-sync changelog to README + comprehensive README update

- Add commit_push.sh: Automated version bump + README changelog sync
- Update sync_changelog.sh: Auto-generate description from git commit message
- Update README feature list: Document dark theme colors, new features (魔法通讯, 日记, 小剧场, 红娘)
- Update README: Document triple-guarantee narrative system
- Update README: Document AI communication system with NPC chat
- Fix color references in README to match dark theme palette

### v0.3.5 — 2026-08-20

**🔧 代码更新**
- 常规代码更新与优化

### v0.3.4 — 2026-08-20

**🔧 代码更新**
- 常规代码更新与优化

### v0.3.3 — 2026-08-20

**🔧 代码更新**
- 实现的功能
 
1. 📱 魔法通讯（完全重写）
 
对接真实NPC数据：从游戏中的NPC注册表自动加载联系人列表
​
学院筛选：按格兰芬多/斯莱特林/拉文克劳/赫奇帕奇/教职工分类
​
好感度显示：每个联系人显示当前好感度等级（死敌→灵魂伴侣）
​
AI智能对话：点击联系人进入 NpcChatScreen
​
使用 AI 服务生成符合NPC性格的回复
​
根据好感度调整语气（友好/冷淡）
​
对话历史持久化保存
​
对话自动影响好感度变化
 
2. 💬 魔法论坛（重写）
 
5个分类（校园八卦/学术讨论/魁地奇/食谱分享/寻人启事）
​
点赞/取消点赞交互
​
发布新帖对话框（选分类+写内容）
​
统一深色主题
 
3. 📖 日记系统（重写）
 
自动从游戏状态生成初始日记条目
​
支持手动写日记，8种心情表情选择
​
标记"系统生成"和"用户手写"条目
​
可删除自己的日记
 
4. 🎭 平行世界·小剧场（新增）
 
5个预置"如果"场景
​
支持收藏场景
​
可创建自定义小剧场（选图标+写标题描述）
 
5. 💞 姻缘一线牵红娘（新增）
 
智能匹配算法：学院+年级+性格重叠+好感度综合评分
​
显示匹配原因（同院情谊/性格互补/高度契合等）
​
可重新分析刷新匹配结果
 
6. 📲 应用商店（新增）
 
"敬请期待"提示对话框


### v0.1.2 — 2026-08-20

**🔧 代码更新**
- 常规代码更新与优化

### v0.1.1 — 2026-08-21

**🗺️ 世界地图大扩展**
- 🌍 **四大区域地图**：霍格沃茨城堡（18 地点）、伦敦（9 地点）、住宅区（8 地点）、霍格莫德村（12 地点）
- 🏰 **对角巷子地图**：16 家真实魔法店铺（古灵阁、奥利凡德魔杖店、韦斯莱魔法把戏坊、丽痕书店、神奇动物商店等）
- ⚫ **翻倒巷子地图**：6 个黑魔法地点（博金·博克古董店、黑魔法市集、幽灵酒馆、毒药铺等）
- 🧭 **层级导航**：大世界 → 区域 → 子区域（对角巷/翻倒巷），面包屑路径返回
- 📍 **分支地图标记**：带 `branch` 标签的地点可进入更深层子地图

**🎨 剧情富文本高亮系统**
- 🌈 **五色高亮渲染器**（`StoryTextRenderer`）：自动识别并着色剧情中的关键元素
  - 👤 **人名**：琥珀橙色（`#B45309`）加粗显示
  - 💬 **对话**：蓝色（`#1565C0`）加粗，支持「」、""、『』等多种引号
  - 📍 **地点**：翠绿色（`#059669`）加粗
  - ⚗️ **物品**：紫色（`#7C3AED`）加粗
  - 📖 **旁白**：原羊皮纸棕色（`#4A3728`）
- 📚 **内置词典**：70+ 人名、50+ 地点、40+ 物品关键词自动识别
- 🧩 **智能 token 拆分**：对话优先识别，避免与关键词冲突

**🔧 CI/CD 修复**
- 🔄 修复自动版本递增后 push 回仓库冲突问题
- 🛡️ 版本递增仅用于构建产物命名（GitHub Actions outputs 传递版本号）
- 📦 APK 重命名规则：`HogwartLige-{版本号}.apk`（如 `HogwartLige-0.1.1.apk`）

### v0.1.0 — 2026-08-21

**🎮 核心玩法增强**
- 👤 **角色属性面板**：容貌/体质/智力/魅力/体能/道德值六大属性进度条
- 💼 **找点活干（职业系统）**：5 种岗位，体力/金币管理，AI 智能推荐
- 📖 **回忆系统**：章节时间线、收藏、CG 画廊
- 🧑‍🤝‍🧑 **NPC 关系网**：14+ 原著角色，动态好感度（敌对→冷淡→初识→朋友→亲密→挚友）

### v0.0.6 — 2026-08-20

**✨ 全新玩法系统**
- 🌍 **世界 Tab 重构**：NPC 卡片显示完整描述文本、关系状态、同地点标签；已登场/未登场折叠分区；收藏引入和新建 NPC 按钮
- 👤 **角色属性面板**：头像+按钮、属性进度条+数值显示、学院/血统标签、位置信息
- 📖 **剧情 Tab**：面板/事件 Tab 切换、左侧菜单按钮、底部胶囊输入栏
- ⚙️ **设置 Tab 增强**：同步剧本/存读档/导入导出按钮
- 📊 **Token 用量统计**：显示本月消耗、预计费用、本地 API Key 直连说明
- 📱 **手机主界面**：实时时钟、玩家头像、音乐播放器、功能图标网格
- 📖 **回忆系统**：章节时间线、收藏、CG 画廊
- 💼 **职业系统**：5 种岗位、体力/金币管理、AI 智能推荐
- 🗺 **世界地图**：霍格沃茨+霍格莫德村地图、地点卡片、Canvas 自定义地图
- 🪄 **魔法通讯/论坛/日记**：联系人列表、分类帖子、日记条目

**🔧 技术改进**
- 支持三模型：DeepSeek / 智谱AI / Agnes-2.5-flash
- 修复 Flutter 3.16 兼容性（withOpacity、CardTheme）
- 自动版本号递增（Patch 满 10 进 Minor，Minor 满 10 进 Major）
- APK 重命名为 HogwartLige-{版本号}.apk
- App 名称改为 HogwartLige

### v0.0.5 — 2026-08-19

**🛠 Bug 修复与优化**
- 修复 Provider 状态管理错误
- 修复 NPC 数据未定义问题
- 修复 UI 组件兼容性问题（Badge、icons、Path API）
- 优化 Provider 存储逻辑

### v0.0.4 — 2026-08-18

**🎨 初始界面实现**
- 底部 4 Tab 导航（剧情/手机/世界/设置）
- 温暖羊皮纸风格主题
- 事件卡片样式实现

---

## 📄 开源协议

本项目仅供学习和交流使用。