...
1000 回合后 1500 字塞不下千件事，重要事实一定会被淘汰。现在改架构：
...
**📋 变更说明**
feat(npc): 为全部57个NPC添加常用缩写/别名匹配，改进好感度解析

- 所有NPC添加aliases字段，收录常用昵称、简称、头衔（如老蜜蜂/老邓/混血王子/大脚板等）
- NPC类新增allNames自动推导（从名·姓拆分姓氏+名字）、nameMatches匹配、nameMatchScore评分
- 移除自动推导的冗余别名（如单独的姓氏/名字）和模糊头衔（如通用院长）
- 好感度解析从首个命中改为最佳匹配评分，避免韦斯莱命中多人时选错人
- 清理后跨角色冲突仅剩2个（双胞胎/捣蛋王，弗雷德&乔治组合，可接受）
...
**📋 变更说明**
fix: 修复精力系统、NPC自动注册、好感度更新和剧情质量

1. 精力系统修复:
   - 消耗从5/3/2降到2/2/1
   - 恢复关键词扩展（睡觉、休息、冥想、散步等）
   - 恢复量从40/20提升到50/30
...
- 添加_generateChoicesSeparately方法：独立调用AI生成选项
- 添加_parseNarrativeOnly方法：只解析叙事文本，过滤选项行
- 修改processChoice：实现两次AI调用流程（叙事+选项分离）
- 添加多级兜底机制：独立生成→原始解析→智能提取→上下文兜底
- 选项现在完全基于已生成的剧情，提升关联性

### v1.3.5 — 2026-08-22

**📋 变更说明**
fix: 修复开局剧情残留问题 - 改进摘要机制和上下文区分

## 问题根因
AI 在中期生成开局场景选项（如"去车站"、"检票"）的根本原因：
1. 摘要机制保留了开局的具体场景描述（如"车站"、"检票"）
2. buildPrompt 没有明确区分"历史背景"和"当前场景"
3. AI 看到摘要中的早期场景描述后，错误地基于这些历史信息生成选项
