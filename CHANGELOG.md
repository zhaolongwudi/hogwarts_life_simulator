...
**📋 变更说明**
docs: 今日全面完善总结报告（v3.5.0-v3.8.0，六轮工作全记录）
...
**📋 变更说明**
feat(ui): 剧情输出面板小说式分段渲染 v3.7.0——段落层次与颜色体系升级

- 正文按空行拆段+四类识别（叙述/对话/内心独白/时间戳），段落级视觉形态：
  叙述首行缩进、对话浅蓝衬底+左侧色条、内心独白浅紫斜体、时间戳金色胶囊
- 时间戳前缀后跟正文自动降级为叙述段，防整段被胶囊吞掉
- 图例新增内心独白色；好感变化卡数值正绿负红加粗（parseAffectionLine）
- 单段短文本保留原整段渲染回退路径
...
**4. 质量**
- 新增 test/story_paragraph_test.dart 16 项（四类分类/降级判定/样式/好感着色/前缀清理），
...
**📋 变更说明**
feat(ui): 指令中心面板——数据驱动、分组模块化的独立指令面板

- 新增 command_center_panel.dart：从 CommandRegistry 自动读取全部指令（56+条），
  按分组模块化展示、可折叠（作弊组默认收起）、实时搜索、一键运行/填参、高频快捷区
- 底部终端按钮接入新面板，删除旧手写硬编码菜单（仅28条且与注册表脱节）
- 参数判定支持 <尖括号> 与 [方括号] 占位符
- 新增 5 项面板测试，全量 1232 项通过
...
**📋 变更说明**
feat: 全方位深度完善 v3.6.0——死亡与坏结局/职业系统/数值堵漏/锚点修复/体验打磨

基于三路并行审计（人生闭环/世界自治/代码质量）：
- 玩家死亡系统：health 可归零，死亡终章+AI终章，不可逆，至交记念；/cheat 无敌免疫
- 三类坏结局：黑化定性（dark≥70&道德<35→被黑暗吞没），/结局 注入死亡/堕落状态
- 毕业后职业系统：8条职业线（傲罗/治疗师/魔法部/记者/魁地奇/诅咒破解/魔药大师/普通），成绩+属性+声望门槛，九月年结发薪晋升，/职业 指令
- 数值堵漏：/课堂 互动 每日3次+精力成本；打工每日2单+年级限制+属性校验

### v3.4.2 — 2026-08-31

**📋 变更说明**
chore: 移除阿尼马格斯 mixin 未使用的 import
...
依据《框架1·霍格沃兹人生模拟器典藏版》与《框架2·高自由度巫师人生模拟》逐节核查：

- 作弊指令从7个子命令补齐至25+（8.1-8.5）：熟练度/加隆/世界线/知晓/剧情/无敌/全知/重置/列表/固定好感/骨科恢复/配对系列(好感·关系·重置·查看·性取向·列表)/声望NPC/收藏/成就/宠物羁绊/新NPC(生成·好感·删除)
- 新增字段：Player.cheatInvincible/cheatOmniscient/cheatOrientationBackup/cheatModifiedPairs/examRecords/patronus，NPC.affectionLocked（老档兼容）
- 阿尼马格斯系统（框架2 §67）：四段式长线（研习→训练→满月夜尝试→登记），形态与人格关联，失败有代价
...
- 四段式长线：研习理论（五年级+魔药/变形≥60）→ 训练进度（5~15/次）→ 药剂就绪 → **每月十五满月夜**尝试（成功率随训练与熟练度上升）→ 魔法部登记（未登记是重罪）
