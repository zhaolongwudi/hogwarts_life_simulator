...
// ==================== 刷分防御 · 上限 ====================
//
// 学院杯的日常加分靠 AI 文本关键词触发（上面那张规则表）。
// 玩家可以反复写"课堂上答对了问题"这种动作去凑词刷分——每回合最多加减
// 一次这条约束挡不住他，因为时间每回合都在走，一天能刷十几个回合。
// 所以这里卡日/周上限：凑再多词，一天也只能领到 15 分、一周 40 分。
// 为什么只卡加分不卡扣分：没人会为了被扣分去刷文案，扣分本来就是负反馈。

/// 叙事关键词路径（日常表现）每日允许的加分上限。
const int kHouseNarrativeGainDailyCap = 15;

/// 叙事关键词路径（日常表现）每周允许的加分上限。
const int kHouseNarrativeGainWeeklyCap = 40;

// ==================== 年度榜 · 基准与对手增长 ====================
//
// 学年榜不是学年末掷一次骰子：四院各从基准分起步，其它三院在学期内按
// 上学日逐日自然增长（世界不因玩家而停转），玩家的学院行 = 基准 + 玩家
// 本学年贡献。玩家能看着榜单一路长起来，也能靠自己的贡献把它翻过去。

/// 四院年度榜的学期起始分。
const int kHouseCupBaseScore = 130;

/// 其它三院每个上学日自然增长的分数区间（含，均匀随机）。
