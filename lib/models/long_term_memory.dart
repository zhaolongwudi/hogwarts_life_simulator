...
int importanceForFact(String fact) {
  // 不可逆的重大变故 / 关系质变：永不淘汰
  //
  // 第九次审查补词：中文写死亡/剧变很少直呼"死了"，AI 叙事尤其爱用
  // 委婉表达——"她牺牲了""他没能回来""从此长眠"。旧表里这类事实
  // 会漏判为 5 分日常流水，随新鲜度衰减到 1.0 分后被淘汰：
  // **最重要的记忆反而最容易丢**。补词原则：只收语义明确、
  // 不会被日常语境误触发的词（故不收"走了""离开"这类歧义词）。
  const critical = [
    '死了', '死亡', '死去', '杀害', '杀死', '谋杀', '遇害', '殉职', '阵亡',
    '牺牲', '离世', '去世', '逝世', '病逝', '战死', '自尽', '自杀',
    '长眠', '没能回来', '咽气', '断气',
    '复仇', '血债', '命债',
    '订婚', '结婚', '婚礼', '婚约', '离婚', '分手', '决裂', '绝交',
    '背叛', '出卖', '告密', '倒戈',
