...
// 注：这里原本有个 ActionTimeCost（设定 9.4 的行动耗时表），从头到尾没被调用过。
// 真正生效的是 lib/data/time_cost_rules.dart —— 那版除了耗时还给每条规则配了
// 关键词和优先级（「禁林」必须优先于「探索」，否则 180 分钟会被 60 分钟抢先命中）。
// 两张表并存只会让人改错地方，旧的这份已删。
