...
> 报告生成：2026-09-07 · 台账整理：2026-09-10（批次 30 后）· 当前版本 4.2.7
...
| 还剩哪些没做 | [§8.2 未处理事项](#82-未处理事项批次-30-已清零) |
...
| 测试代码行数 | 16,600 |
| 审查维度 | 38 |
| 问题台账 | 78 条（✅ 48 闭环 · 🟢 28 收口 · 🟡 1 部分 · 🟗 1 重新定性 · ⬜ 0 未处理） |
| 最大文件行数 | 3,234 |
| 核心 Provider | 8 |
| Mixin 数 | 14 |
| AI 服务 | 3 |
...
> **台账 78 条：✅ 48 完全闭环 · 🟢 28 已收口 · 🟡 1 部分 · 🟗 1 重新定性 · ⬜ 0 未处理**（批次 30 收尾：F41/P2/P3 核对收口，F44/L1/L2/L3 定案不做）
...
| 30 | 未处理条目收尾：F41/P2/P3 核对收口，F44/L1/L2/L3 定案不做，P1/CS3 详情同步 | F41 / P2 / P3 / F44 / L1 / L2 / L3 | ✅ 已推送（纯文档批次：逐条核对代码后同步详情段与总表；台账 ⬜ 7 → 0） |
...
| F41 | mixin 间存在隐式通信 | Mixin 架构 | Medium | v2 | 🟢 批次30（核对收口：12 个 mixin 全部 `on GameProviderBase`，`on` 子句即显式契约） |
...
| F44 | 图片格式不统一，加载策略单一 | 资源管理 | Low | v2 | ✅ 批次30（定案不做：17 张本地小图合计 516KB，webp 转换与网络图缓存均无收益对象） |
...
| P2 | story_text_renderer 渲染性能瓶颈 | 性能基准 | High | v3 | 🟢 批次30（核对收口：正则已全部 static final（F30/批次5/8），colon 死循环双边界已修复，与 F30 同源） |
| P3 | 频繁的集合重建 | 性能基准 | Medium | v3 | 🟢 批次30（核对收口：全文件仅剩 1 处 `toList()..sort()`，"10+ 次"存量已随 F32/批次5 静态化消除；collection 基准已建基线） |
...
| L1 | 缺少延迟加载 | 延迟加载 | Medium | v3 | ✅ 批次30（定案不做：与 CS2 同误判——const 顶层集合懒初始化；assets 约 1MB 无远程资源） |
| L2 | 图片无懒加载 | 延迟加载 | Medium | v3 | ✅ 批次30（定案不做：全库仅 1 处 `Image.asset`，本地 asset 解码异步、17 张合计 516KB） |
| L3 | screen 级别无懒加载 | 延迟加载 | Low | v3 | ✅ 批次30（定案不做：deferred import 面向 Web 包体，Android 单机 APK 无收益） |
...
> 12 个业务 mixin 全部声明 `on GameProviderBase`（如 `GameResponseAffectionMixin on
> GameProviderBase, GameResponseChoiceMixin`），`on` 子句就是**显式契约**——mixin 只能
> 访问 base 公开的成员，混入顺序与依赖关系在声明处可见，与 F40 批次 11c 的组织评估一致
> （混合式组合对单例状态容器利大于弊）。「隐式通信」在单例状态容器架构下不存在：
> 所有跨 mixin 数据交换都经过 `GameProviderBase` 这一个显式边界。
...
> 核对事实：`assets/images/avatars/` 共 17 张本地小图（最大 127KB，目录合计 516KB），
> 全部随 APK 打包、无任何网络图片。webp/avif 转换对 516KB 的收益趋近于零；
> `cached_network_image` 是网络图缓存方案，本地 asset 场景不适用。
> 单机自用场景下维持现状即正确（与 F23 / PC1 同一产品立场）。
...
> 暗色品牌启动页：`launch_background.xml` 深色背景 + 居中图标，`values-night/` 与
> `values-v31/`（Android 12+ 系统启动屏 icon）分版本适配，`styles.xml` 配
> `NormalTheme` 防白闪。详见 [批次 29 记录](#批次-29--性能基准起步--去-barrel-化--启动画面--平台国际化定案)。
...
> 与 CS2 同一误判：`npc_data.dart` 全是 `const` 顶层集合，Dart 顶层变量**懒初始化**
> （首次引用才求值），`const` 更在编译期规范化——"启动时全部加载"不存在。
> 资源侧：`assets/images/` 合计约 1MB 本地打包资源，无远程资源，全量加载也就
> 一次内存映射的规模。单机场景没有延迟加载的收益对象。
...
> 头像加载全库只有一处 `Image.asset`（`lib/widgets/npc_avatar.dart`）。本地 asset
> 的解码本身就是异步的（不阻塞 UI 线程），且 17 张图合计 516KB、单张最大 127KB，
> 加载耗时在几毫秒量级。`FadeInImage`/占位图是为网络图与慢解码场景设计的，
> 本地小图套占位图反而增加无意义的状态切换。
...
> Dart 的 `deferred import` 主要收益是减小 Web/JS 首包体积。本仓库是 Android 单机
> APK（安装包整体下载，无首包/按需拉取概念），deferred 化只会增加 `loadLibrary()`
> 的异步复杂度与调用点错误处理，收益为零。路由统一已由 F28/F29 批次 10 收口，
> screen 均按需 `pushRoute` 构建，没有"导航时全量预构建"的问题。
...
**状态**：🟢 批次 29（部分落地，详见批次 29 记录）

> `benchmark_harness`（dart-lang 官方）+ 两个纯 Dart 基准：
> `benchmark/serialization_benchmark.dart`（Player toJson/fromJson 往返吞吐基线）、
> `benchmark/collection_rebuild_benchmark.dart`（收藏集合重建开销基线）。
> 渲染帧率与启动耗时需要真机 profiling（本地 CI 无设备），登记为缓做项。
...
> 逐项核对（与 F30/F32 同源，批次 5/8 已静态化）：
> - 30+ 正则全部为 `static final RegExp` 类级常量（含标签/动词/记号预编译），
>   每次渲染零编译开销；
> - colon 死循环已有修复：`_parseColonSegments` 内
>   「BUG-CRASH 修复：nameEndInRaw 转 absolute 后不能超过 k」+
>   「k+1（colonEnd）不能超 safeMax」双边界，且每个分支 `advanceLine()` 必推进防死循环；
> - 渲染管线正确性由 `narrative_format_test` 等 38 个测试文件钉住。
> 长文本渲染卡顿无真机复现记录，登记为需设备 profiling 验证的缓做项。
...
> 全文件现仅剩 **1 处** `toList()..sort()`（按角色名长度降序的贪心匹配），
> 其余重复重建在批次 5/8（F30/F32）已随正则静态化一并消除——"10+ 次"是
> 报告生成时的存量，现不复存在。且批次 29 的
> `benchmark/collection_rebuild_benchmark.dart` 已为集合重建建立基准基线，
> 后续可自动发现回归。
...
> 产品决策：仅 Android、单机自用（用户唯一、无多平台受众），多平台没有收益对象。
> 维持现状即正确，与 F23 / F44 / L1 / L2 / L3 同一产品立场。
...
> 跟随 PC1：无多平台目标就不存在「缺条件编译」问题；单 Android 目标下 `dart:io`
> 路径恒成立。若将来外发多平台再重新开项。
...
> 跟随 PC1：iOS 无目标平台。Android 签名配置模板已由 F27 / 批次 1 提供
> （`key.properties` + `signingConfigs` 模板），本条目实为误判的一部分。
...
> 原路线图按报告生成时的优先级（P0–P3）罗列建议，随着批次 1–30 推进已严重过时：
...
| 状态 | 数量 | 含义 |
|---|---|---|
| ✅ 完全闭环 | 48 | 已修复 / 误判更正 / 核对关闭 / 定案不做 |
| 🟢 已收口 | 28 | 主体完成；剩余部分在条目里注明边界或缓做理由 |
| 🟡 部分完成 | 1 | CS1（剩余「runApp 提前」定案缓做，见 §6 · CS1） |
| 🟗 重新定性 | 1 | F20（不再以扫描清零为目标，转为长期维护约束） |
| ⬜ 未处理 | 0 | 批次 30 已全部处置 |

合计 78 条，**全部有结论**：做完的做完、收口的收口、定案的不做、误判的更正。
剩余缓做项（真机 profiling 类）均在条目内注明触发条件。

### 8.2 未处理事项（批次 30 已清零）

批次 30 后台账 ⬜ = 0。原 14 项的去向：

| 条目 | 去向 |
|---|---|
| P1 / P2 / P3 | 🟢 批次 29/30 收口：基准落地 + 核对（正则静态化、集合重建收敛、死循环修复）；渲染帧率与启动耗时登记为真机 profiling 缓做项 |
| F23 | ✅ 批次 29 定案不做（单机中文用户） |
| F41 | 🟢 批次 30 核对收口（`on GameProviderBase` 即显式契约） |
| L1 / L2 / L3 | ✅ 批次 30 定案不做（资源 ~1MB 本地打包、无远程、无按需拉取场景） |
| PC1 / PC2 / PC3 | ✅ 批次 29 定案不做（仅 Android、单机自用） |
| CS3 | ✅ 批次 29 已修复（暗色品牌启动页 + 12+ 系统启动屏） |
| F38 | ✅ 批次 29 已修复（删除 2 个单消费者 barrel） |
| F44 | ✅ 批次 30 定案不做（17 张本地小图 516KB，无收益对象） |
...
| F44 | 17 张本地小图合计 516KB、无网络图，webp 转换与 `cached_network_image` 无收益对象 | 批次 30 |
| L1 | 与 CS2 同误判：const 顶层集合懒初始化；assets 约 1MB 无远程资源 | 批次 30 |
| L2 | 全库仅 1 处 `Image.asset`，本地 asset 解码异步，占位图无收益 | 批次 30 |
| L3 | deferred import 面向 Web 首包，Android 单机 APK 无按需拉取概念 | 批次 30 |
...
### 批次 29 — 性能基准起步 + 去 barrel 化 + 启动画面 + 平台/国际化定案（P1 / F38 / CS3 / PC1 / PC2 / PC3 / F23）

**改动清单**

| 文件 | 改动 |
|---|---|
| `benchmark/serialization_benchmark.dart` | **新增**。`benchmark_harness` 基准：Player `toJson/fromJson` 往返吞吐基线（JSON 用真实字段结构构造） |
| `benchmark/collection_rebuild_benchmark.dart` | **新增**。收藏集合重建开销基线（P3 主题的度量锚点） |
| `pubspec.yaml` | dev_dependencies 加 `benchmark_harness: '>=2.3.0 <3.0.0'`（dart-lang 官方） |
| `lib/mixins/game_provider_mixins.dart` | **删除**。13 个 mixin 的 barrel 文件，单消费者 |
| `lib/screens/other/other_screens.dart` | **删除**。6 个 screen 的 barrel 文件，单消费者 |
| `lib/providers/game_provider.dart` | barrel import 改为 13 个 mixin 直接 import（F38） |
| `lib/screens/game/game_phone_tab.dart` | barrel import 改为 6 个 screen 直接 import（F38） |
| `android/app/src/main/res/drawable/launch_background.xml` | 暗色品牌启动页 + 居中图标（CS3） |
| `android/app/src/main/res/values/styles.xml` | `NormalTheme` 防白闪 |
| `android/app/src/main/res/values-night/styles.xml` | 暗色主题适配 |
| `android/app/src/main/res/values-v31/styles.xml` | **新增**。Android 12+ 系统启动屏（windowSplashScreen 图标） |
| `android/app/src/main/res/values/colors.xml` | **新增**。启动页颜色定义 |
| `.github/workflows/android-build.yml` | **版本号防膨胀门禁**：仅 `lib/assets/shaders/android/pubspec` 变更才 bump+构建+发 Release（PC1 系列） |
| `框架对照完善报告.md` | 追加「附录二：v4.x 框架对照状态更新」 |

**定案记录**（本批三项"定案不做"均写进对应条目）：
- **F23 国际化**：单机中文用户，收益配不上引入成本；
- **PC1/PC2/PC3 多平台**：仅 Android、单机自用，多平台没有受众，维持现状即正确。

**验证**：本地 `flutter analyze --no-fatal-warnings --no-fatal-infos` → 0 error；
`flutter test` → 1384 passed。推送后 CI run 全绿，但**版本号门禁在首个实战提交即误判**（见 29.1）。

### 批次 29.1 — CI 版本号门禁误判修复（PC1）

**问题**：批次 29 的提交（`5cb5bdb`，含 lib/android 关键路径改动）触发 CI 后，
「Bump version for build」「Build APK」「Create Release」全部 skipped——
门禁把 release-worthy 提交误判成了纯文档提交（run 34402831753）。

**根因**：门禁初版从 `github.event.commits` 的 `added/modified/removed` 提取变更文件，
实测该 payload 在本 workflow 的 push 事件下提取为空，`CHANGED` 为空 → 判定 false。

**修复**：判定依据改为 checkout 后仓库内真实 diff：

```yaml
BEFORE="${{ github.event.before }}"
if [ -z "$BEFORE" ] || [ "$BEFORE" = "0000000000000000000000000000000000000000" ]; then
  RELEASE_WORTHY=true   # 分支首次推送无从 diff，保守视为 release-worthy
else
  CHANGED=$(git -c core.quotepath=false diff --name-only "$BEFORE" HEAD | sort -u)
  # case 匹配 lib/* assets/* shaders/* android/* pubspec.yaml(.lock) → RELEASE_WORTHY=true
fi
```

不依赖事件 payload 结构，本地验证 16 个变更文件正确命中关键路径。

**验证**：
- `b81f526`（仅 .github/ 变更）→ 门禁判定不 release-worthy：bump/APK/Release 全 skipped，
  版本号未消耗，analyze+test 全绿 —— 门禁的"省版本号"路径验证通过；
- workflow_dispatch 手动触发完整构建 → 版本 `4.2.6+426 → 4.2.7+427`，APK + Release v4.2.7
  + changelog 同步（`fd4b674`，带 `[skip ci]` 不触发循环）全部成功 —— 构建链路验证通过。

### 批次 30 — 未处理条目收尾（F41 / P2 / P3 / F44 / L1 / L2 / L3）

**这一批的由来**：台账里还剩 7 条「⬜ 未处理」。逐条回到代码核对真实状态，
发现其中 4 条**实质早已闭环**（文档滞后），3 条**评估后应定案不做**。纯文档批次，
不触碰 Dart 逻辑。
...
| 条目 | 核对事实 | 结论 |
|---|---|---|
| **F41** mixin 隐式通信 | 12 个业务 mixin 全部声明 `on GameProviderBase`（含 2 个额外 `on` 依赖），`on` 子句即显式契约，跨 mixin 数据交换都经 base 一个显式边界 | 🟢 已收口（核对更正） |
| **P2** renderer 性能瓶颈 | 30+ 正则全部 `static final`（F30/批次5/8 成果）；colon 死循环已有 BUG-CRASH 双边界修复 + 每分支 `advanceLine()` 必推进；无真机卡顿复现记录 | 🟢 已收口（核对更正） |
| **P3** 频繁集合重建 | 全文件仅剩 1 处 `toList()..sort()`，"10+ 次"存量已随 F32/批次5 消除；批次 29 已建 collection 基准基线 | 🟢 已收口（核对更正） |
| **F44** 图片格式不统一 | 17 张本地小图合计 516KB（最大 127KB），无网络图；webp 转换与 `cached_network_image` 均无收益对象 | ✅ 定案不做 |
| **L1** 缺少延迟加载 | 与 CS2 同误判：const 顶层集合懒初始化；assets 约 1MB 无远程资源 | ✅ 定案不做 |
| **L2** 图片无懒加载 | 全库仅 1 处 `Image.asset`（npc_avatar），本地 asset 解码异步、几毫秒量级 | ✅ 定案不做 |
| **L3** screen 级懒加载 | deferred import 面向 Web/JS 首包，Android 单机 APK 无按需拉取概念 | ✅ 定案不做 |

**文档同步**：详情段 ×9（含 P1/CS3 详情滞后同步）+ 总表状态列 + 台账统计行
（✅ 44→48 · 🟢 25→28 · ⬜ 7→0，总数 78 不变）。

**验证**：纯文档变更，本地 `flutter analyze` / `flutter test` 结果不受影响；
推送后 CI 跑 analyze+test（门禁判定不 release-worthy，版本号不消耗）。
...
> **全方位无遗漏审查报告 v3 — 终极版**  
> Hogwarts Life Simulator &copy; 2026 | 三轮审查覆盖 38 个维度，问题台账 78 条（已处置 78 / 未处理 0）  
> 审查工具：Trae Work | 报告生成日期：2026-09-07 | 修复台账整理：2026-09-10（批次 30 后）
