...
本地无-SNI TLS 终止代理：让 git/gh 在沙箱里也能连 GitHub。
  ...
  2) 本代理在 127.0.0.1:443 监听 HTTPS（用本地 CA 签发的叶子证书）；
  3) git 把请求发本代理，代理解密后以「无 SNI」直连 GitHub 真实 IP 回源；
  4) git 通过 GIT_SSL_CAINFO 信任本地 CA。
出口流量不带 SNI，绕开 DPI；认证照常（Authorization 头原样转发）。
"""
import socket, ssl, threading, sys

LISTEN_HOST, LISTEN_PORT = "127.0.0.1", 443
CA_CRT = "ca.crt"; LEAF_CRT = "leaf.crt"; LEAF_KEY = "leaf.key"
...
def ips_for(host):
    ...
    if h == "api.github.com" or h.endswith(".api.github.com"):
        return API_IPS
    return WEB_IPS   # github.com / *.github.com / raw / codeload / objects 均走 web 节点
...
def read_until(sock, delimiter=b"\r\n\r\n", maxlen=65536):
    buf = b""
    while delimiter not in buf and len(buf) < maxlen:
        c = sock.recv(4096)
        if not c: break
        buf += c
    return buf
    ...
    try:
        ...
        for l in lines[1:]:
            if l.lower().startswith(b"host:"):
                host = l.split(b":", 1)[1].strip().decode().split(":")[0]
            if l.lower().startswith(b"content-length:"):
                cl = int(l.split(b":", 1)[1].strip())
        _, _, already = head.partition(b"\r\n\r\n")
        body = already
        while len(body) < cl:
            c = client.recv(4096)
            if not c: break
            body += c
        body = body[:cl]
        sys.stderr.write(f"[proxy] {method} {host}{path} (body={cl})\n")
        up = upstream_connect(host)
        up.sendall(head + body)
        resp_head = read_until(up)
        ...
        sys.stderr.write(f"[proxy] err: {repr(e)}\n")
    finally:
        try: client.close()
...
def main():
    ...
    srv.bind((LISTEN_HOST, LISTEN_PORT)); srv.listen(64)
    sys.stderr.write(f"[proxy] listening https://{LISTEN_HOST}:{LISTEN_PORT}\n")
    while True:
        c, _ = srv.accept()
        try:
            tc = ctx.wrap_socket(c, server_side=True)
        except Exception as e:
            sys.stderr.write(f"[proxy] tls err: {repr(e)}\n"); c.close(); continue
...
- `git clone --depth 1` ✅ 传输层正常（本仓库含 `hogwarts-zip` 等较大标签，完整 clone 较慢，非传输问题）。
