...
| 批次 | 状态 | commit | CI |
|---|---|---|---|
| S1 | ✅ 完成 | `62449d1`（+CI 修复） | run 35767275523 全绿 |
| S2+S3+S12 | ✅ 完成（减法三连） | `bedd7d3` | run 35769451843 全绿 |
| S4+S7 | ✅ 完成（睡眠语义两表对齐 + 魁地奇训练 30 分钟） | `f5fd35a` | run 35771088561 全绿 |
| S5 | ✅ 完成（奇遇结算写 happenstanceLog + 上限 50） | `b62369a` | run 35772018030 覆盖验证 |
| S6+S13 | ✅ 完成（决斗跳档逐档补发 + 四社专项测试 9 用例） | `1bb0a53` | run 35772018030 全绿 |
| S8+S9+S10+S11 | ✅ 完成（赛季积分递减 / 银色鳞片进八眼巨蛛掉落 / 快讯学期口径 / 魁地奇周重置） | `6d0ed04` | run 35774061430 全绿 |
| S14 | ✅ 完成（5 设计文档状态行 + README 命令表 + 台账补批次 B/C + BATCH10 HEAD） | `e3e8a14` | run 35774924302 验证中 |
| **S15** | ⏸ **评估后搁置**（理由见下） | — | — |

**S15 搁置理由（2026-09-23 评估结论）**：
- 涉及 `generateChoicesSeparately`（mixin_response.dart:2033，**476 行**）的合并改造，
  历史 BUG-H（模型返回选项而非叙事）/ BUG-L（选项夹带过期内容）复发风险高；
- 选项端独立 prompt（末尾 800 字 + 玩家硬状态 + NPC 关系 + 停滞检测 + 禁止词 + OOC 软提醒 + 断言块）
  压缩进叙事 prompt 后选项质量大概率下降；
- S1 流式已解决"反应慢"主因，S15 收益（省第二次调用等待）边际递减；
- 与本项目"减法优先、不过度工程化"方向相悖；
- **若后续要执行**：单独一批、充分测试，先跑 batch8_narrative_prompts_test 与选项相关全部测试。
